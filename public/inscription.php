...
if($_POST)
{
    ...
    {
        $membre = executeRequete("SELECT * FROM membre WHERE pseudo='$pseudo' OR email = '$email'");
        if($membre->num_rows > 0)
        {
            $contenu .= "<div class='erreur'>Un compte existe déjà. Veuillez vérifier votre email ou choissir un autre pseudo.</div>";
        }
        else
        {
            ...
            executeRequete("INSERT INTO membre (pseudo, mdp, nom, prenom, email, civilite, ville, code_postal, adresse) VALUES ('$pseudo, '$mdp', '$nom', '$prenom', '$email', '$civilite', '$ville', '$code_postal', '$adresse')");
